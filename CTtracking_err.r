...
pairup <- function(dat, pairtag){
  ...
  dat$pair_id <- tidyr::unite(dplyr::select(dat, pairtag), "pr", sep="/")$pr
...
#   displacement_err: estimated linear distance moved with a error term common for all 
#   displacement_err_new: estimated linear distance moved with deployment-wise error term
...
seq.data <- function(dat){
  ...
  displacement_err <- coseqn(dat$radius_err[-nrow(dat)], dat$radius_err[-1], d.angle[-1])
  displacement_err_new <- coseqn(dat$radius_new[-nrow(dat)], dat$radius_new[-1], d.angle[-1])
  cbind(dat, imgcount=imgcount, pixdiff=pixdiff, displacement=c(NA,displacement), 
        displacement_err=c(NA, displacement_err),
        displacement_err_new=c(NA, displacement_err_new), d.angle=d.angle)
}
...
seq.summary <- function(dat, datetimetag="DateTimeOriginal", tformat="%Y:%m:%d %H:%M:%S",
  ...
  calc.mov <- function(dat){
    ...
    mvdist_err <- with(dat, tapply(displacement_err, sequence_id, sum, na.rm=T) )
    mvdist_err_new <- with(dat, tapply(displacement_err_new, sequence_id, sum, na.rm=T) )
    ...
    cbind(dat[dat$imgcount==1, !(names(dat) %in% c("imgcount","pixdiff","displacement","d.angle", 
                                                   "displacement_err","displacement_err_new"))],
          pixdiff=pixdiff,
          dist=mvdist,
          dist_err=mvdist_err,
          dist_err_new=mvdist_err_new,
          timediff=mvtime,
          time=time,
          speed=mvdist/time,
          speed_err=mvdist_err/time,
          speed_err_new=mvdist_err_new/time,
          frames=n
    )
  }
